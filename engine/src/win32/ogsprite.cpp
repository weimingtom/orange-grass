...
#ifdef WIN32
	MatrixOrthoRH(m_mOrthoProj, SCR_WIDTH, SCR_HEIGHT, -1, 1, false);
#else
	MatrixOrthoRH(m_mOrthoProj, SCR_HEIGHT, SCR_WIDTH, -1, 1, true);
#endif
}
...
void COGSprite::SetPosition (float _fX, float _fY, float _fWidth, float _fHeight)
{
	...
	m_Vertices[0] = -fHalfScrWidth+m_fWidth+m_fX;	m_Vertices[1] = fHalfScrHeight-m_fHeight-m_fY;	m_Vertices[2] = 0.0f;  m_Vertices[3] = 1.0f;  m_Vertices[4] = 0.0f;
	...
	m_Vertices[15] = -fHalfScrWidth+m_fX;			m_Vertices[16] = fHalfScrHeight-m_fY;			m_Vertices[17] = 0.0f; m_Vertices[18] = 0.0f; m_Vertices[19] = 1.0f;
