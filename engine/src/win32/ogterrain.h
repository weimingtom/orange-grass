...
// Convert coords to index
inline unsigned short COGTerrain::CoordsToId ( int _x, int _y )
{
	return _y * m_Width + _x;
}

// Check coordinate bounds
inline bool COGTerrain::CheckCoordBounds ( int _x, int _y )
{
...
// World to grid position X
inline int COGTerrain::WorldToGrid_X ( const Vec3& _vPos )
{
	return (int)((_vPos.x - m_vPosition.x) / COGTerrain::g_fStep);
}


// World to grid position Y
inline int COGTerrain::WorldToGrid_Y ( const Vec3& _vPos )
{
	return (int)((_vPos.z - m_vPosition.z) / COGTerrain::g_fStep);
}
