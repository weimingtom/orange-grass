...
const int COGTerrain::g_Width = 128;
const int COGTerrain::g_Height = 512;
const float COGTerrain::g_fStep = 17.0f;
const float COGTerrain::g_fHeightScale = 1.4f;
const float COGTerrain::g_fMaxHeight = COGTerrain::g_fHeightScale * 255.0f;
const float COGTerrain::g_fAttentuation = 0.5f;
const Vec3 COGTerrain::g_BaseColor (0.5f, 0.8f, 0.1f);
...
// Save the terrain.
bool COGTerrain::Save (const char* _pPath)
{
	int ChunkSize = 128;
...
// Set terrain texture.
void COGTerrain::SetTexture (IOGTexture* _pTexture)
{
...
void COGTerrain::GenerateFaceNormal (GLushort i1, GLushort i2, GLushort i3)
{
...
void COGTerrain::UpdateVertexNormal ( int _id, const Vec3& _updatedVec )
{
...
// Update vertex normal
void COGTerrain::UpdateVertexNormal ( int _x, int _y, const Vec3& _updatedVec )
{
...
COGTerrain::TVertex* COGTerrain::GetVertex (const Vec3& _vPos)
{
...
// Get vertex from a heighmap grid
COGTerrain::TVertex* COGTerrain::GetVertex (int _x, int _y)
{
	if (!CheckCoordBounds(_x, _y))
		return NULL;
...
void COGTerrain::AddHeight (int _x, int _y, float _v)
{
...
void COGTerrain::AddHeight (const Vec3& _vPos, int _patchsize, float _v, bool _bSmoothen)
{
	int x = WorldToGrid_X(_vPos);
	int y = WorldToGrid_Y(_vPos);
	if (!CheckCoordBounds(x, y))
...
int COGTerrain::GetPatch (const Vec3& _vPos, int _patchsize, Vec3* _pOut)
{
	int x = WorldToGrid_X(_vPos);
	int y = WorldToGrid_Y(_vPos);
	if (!CheckCoordBounds(x, y))
...
void COGTerrain::SmoothenPatch (const Vec3& _vPos, int _patchsize)
{
	int x = WorldToGrid_X(_vPos);
	int y = WorldToGrid_Y(_vPos);
	if (!CheckCoordBounds(x, y))
...
void COGTerrain::UpdateNormals ( bool _bUpdateIndices )
{
...
void COGTerrain::ChangeColor (const Vec3& _vPos, int _patchsize, const Vec3& _color)
{
	int x = WorldToGrid_X(_vPos);
	int y = WorldToGrid_Y(_vPos);
	if (!CheckCoordBounds(x, y))
...
void COGTerrain::ChangeBaseColor (int _x, int _y, const Vec3& _color, float _fFactor)
{
